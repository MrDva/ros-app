...
public class LogFragment extends Fragment {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        List<Topic> topicList = rosDomain.getTopicList();
        ...
        rosDate.observe(getViewLifecycleOwner(),rosData -> {
           ...
           /*Set<String> subSet = sharedPreferences.getStringSet(SUB_CONTENT, new HashSet<>());
            ...
            String msg="null";
            if(rosData.getTopic().name.equals(sharedPreferences.getString(TopicName.MAP,TopicName.MAP))&&isChecked[0]){
            ...
            }else if(rosData.getTopic().name.equals(sharedPreferences.getString(TopicName.IMU,TopicName.IMU))&&isChecked[2]){
                ...
                subList.add(sdf.format(System.currentTimeMillis())+"  --  topicName:"+rosData.getTopic().name+"  type:"+rosData.getTopic().type+"  msg:"+msg+"\n");
            }else if(rosData.getTopic().name.equals(sharedPreferences.getString(TopicName.BATTERY,TopicName.BATTERY))&&isChecked[1]){
                BatteryState batteryState=(BatteryState) rosData.getMessage();
                msg="BatteryState:["+batteryState.getPercentage()+"]";
                subList.add(sdf.format(System.currentTimeMillis())+"  --  topicName:"+rosData.getTopic().name+"  type:"+rosData.getTopic().type+"  msg:"+msg+"\n");
            }else if(rosData.getTopic().name.equals(sharedPreferences.getString(TopicName.RPY,TopicName.RPY))&&isChecked[3]){
                Vector3 vector3=(Vector3) rosData.getMessage();
                msg="RPY:["+"x:"+vector3.getX()+",y:"+vector3.getY()+",z:"+vector3.getZ()+"]";
                subList.add(sdf.format(System.currentTimeMillis())+"  --  topicName:"+rosData.getTopic().name+"  type:"+rosData.getTopic().type+"  msg:"+msg+"\n");
            }else if(rosData.getTopic().name.equals(sharedPreferences.getString(TopicName.TEMPERATURE,TopicName.TEMPERATURE))&&isChecked[4]){
                Temperature temperature=(Temperature) rosData.getMessage();
                msg="Temp:["+temperature.getTemperature()+"]";
                subList.add(sdf.format(System.currentTimeMillis())+"  --  topicName:"+rosData.getTopic().name+"  type:"+rosData.getTopic().type+"  msg:"+msg+"\n");
            }else if(rosData.getTopic().name.equals(sharedPreferences.getString(TopicName.DEST_YAW,TopicName.DEST_YAW))&&isChecked[5]){
        ...
        buttonEdit.setOnClickListener(v->{
            if(isChecked.length<=0){
                Toast.makeText(getContext(),"没有可订阅的主题，请检查是否连接上Master或Master是否有发布的主题",Toast.LENGTH_SHORT);
                return;
            }
